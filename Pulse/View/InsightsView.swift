...
/// Structured output generated by Apple Intelligence when analysing log entry patterns.
/// The three arrays map directly to the three sections shown in ``InsightsView``.
@available(iOS 26.0, *)
@Generable
...
/// AI-powered insights view (iOS 26+) that uses Apple Intelligence to surface behavioural
/// patterns from log entries. Results are cached in `UserDefaults` so the view can display
/// them without re-running the model on every appearance.
@available(iOS 26.0, *)
struct InsightsView: View {
    ...
    /// All log entries across all days, sorted oldest-first.
    private var allLogEntries: [DailyLogEntry] {
        allEntries.flatMap { $0.logEntries ?? [] }.sorted { $0.timestamp < $1.timestamp }
    }

    /// Log entries filtered to the currently selected time window (`selectedDays`).
    /// When `selectedDays` is 0, returns all entries.
    ...
    /// The insights to display: prefers live streaming results over cached ones.
    ...
    /// Removes both the live and cached insights and clears the `UserDefaults` cache.
    ...
    /// Runs the Apple Intelligence analysis on `filteredLogEntries`, streams partial results
    /// into `insights`, and persists the final result to `UserDefaults`.
